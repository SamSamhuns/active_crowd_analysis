...
def _draw_single_box(image, xmin, ymin, xmax, ymax, color=(0, 255, 0), display_str=None, font=None, width=2, alpha=0.5, fill=False):
    ...
    draw.ellipse([(left, top), (right, bottom)], outline=color, fill=alpha_color if fill else None, width=width)
                ...
                if defects is not None and centroid is not None: # Pointing detected
                    ...
                    if(len(boxes) != 0):
                        ...
                        for i, box in enumerate(boxes):  #(xmin, ymin, xmax, ymax)
                            if (box[0] < finger_tip[0] < box[2] and box[1] < finger_tip[1] < box[3]): # bbox contains finger position        
                                if scores[i] > best_score: # best score
                                    best_score = scores[i]
                                    id_final = i

                        if (boxes[id_final][0] < finger_tip[0] < boxes[id_final][2] and boxes[id_final][1] < finger_tip[1] < boxes[id_final][3] and scores[id_final]>0.2):

                            drawn_image = draw(frame, boxes[id_final], labels[id_final], scores[id_final], class_names).astype(np.uint8)

                            #cv2.imshow("frame", drawn_image)

                        else: continue

                cv2.circle(drawn_image, (finger_tip[0], finger_tip[1]), 1, (255, 0, 0), 16)
