...
def convert_locations_to_boxes(locations, priors, center_variance, size_variance):
    ...
    # for loc_batch in locations:
    #     for i, bbox in enumerate(loc_batch):
    #         print(loc_batch[i])
    #         # duplicate pred width to height
    #         loc_batch[i] = np.array(loc_batch[i])
...
def iou_of(boxes0, boxes1, eps=1e-5):
    ...
    Args:
        boxes0: gt (1, num_targets, 3)
        boxes1: predictions (num_priors, 1, 3)
    ...
    # print("overlap left top: " + str(overlap_left_top.shape))
...
def iou_of_circles(boxes0, boxes1, eps=1e-5):
    """
    boxes0: gt (1, num_targets, 3)
    boxes1: predictions (num_priors, 1, 3)
    return IOU(num_priors, num_targets, 1)
    Calculates the iou of two center form circles (x, y, r).
    return size: num_priors x num_targets
    """
    r0s = boxes0[..., 2]
    r1s = boxes1[..., 2]
    areas0 = r0s * r0s
    ...
    # print("utils anchor boxes shape: " + str(boxes1.shape))
    dPosition = boxes0[..., :2] - boxes1[..., :2]
    # print("dposition shape:" + str(dPosition.shape))
    distances = torch.norm(dPosition, dim=2)
    # print("distances shape:" + str(distances.shape))
    overlap_areas = torch.zeros(distances.shape)
    # overlap_areas = overlap_areas.reshape(distances.shape)
    # distances[distances >= r0s + r1s] = 10000000000
    a0s = areas0
    a1s = areas1
    ...
    # other overlap areas are initted as 0\
    # mark circles with meaningful (non-zero) overlaps
...
def assign_priors(
    ...
    boxes = center_form_gt_circles[best_target_per_prior_index]
    return boxes, labels
