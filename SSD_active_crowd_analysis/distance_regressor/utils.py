...
def load_model_weight(model,
                      device,
                      latest_wt_file=None,
                      model_wt_src_dir='distance_regressor/outputs'):
    if latest_wt_file is not None:
        model.load_state_dict(torch.load(latest_wt_file,
                                         map_location=device))
    else:
...
def get_X_y(csv_src='distance_regressor/data/annotations.csv'):
    ...
    y_feat = data[data['class'] == 'Pedestrian'][['zloc'
...
def load_standardizer(standardizer, src='distance_regressor/outputs'):
    standardizer.means = np.load(
        open(os.path.join(src, "standard_means.npy"), "rb"))
    standardizer.stds = np.load(
        open(os.path.join(src, "standard_stds.npy"), "rb"))
    return standardizer
...
def regression_metrics_at_different_dist_levels(model, device, X, y, metric):
    ...
    for d in range(vmin, vmax, step):
        ...
        start_dist.append(d)
        metric_overtime.append(metric(torch.Tensor(y_pred),
                                      torch.Tensor(y_filter)))
