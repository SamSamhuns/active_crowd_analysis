...
def train_distance_regr(
    annot_path="distance_regressor/data/annotations.csv", train_val_npy_path=None
):
    """
    ...
    if train_val_npy_path is None:
        X_train, X_val, y_train, y_val = train_test_split(
            X_feat, y_feat, test_size=0.20, random_state=1
        )
    else:
        X_train, X_val, y_train, y_val = (
            np.load(os.path.join(train_val_npy_path, "X_train.npy")),
