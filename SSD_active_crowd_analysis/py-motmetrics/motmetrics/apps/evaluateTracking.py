...
string in the seqmap.""",
    ...
    parser.add_argument(
        "seqmap", type=str, help="Text file containing all sequences name"
    )
    parser.add_argument(
        "--log",
        type=str,
...
def main():
    ...
    gt = OrderedDict(
        [
            (
                seqs[i],
                (
                    mm.io.loadtxt(f, fmt=args.fmt),
                    os.path.join(args.groundtruths, seqs[i], "seqinfo.ini"),
                ),
            )
    ...
    ts = OrderedDict(
        [(seqs[i], mm.io.loadtxt(f, fmt=args.fmt)) for i, f in enumerate(tsfiles)]
    )

    mh = mm.metrics.create()
    st = time.time()
    accs, analysis, names = compare_dataframes(gt, ts, args.log, 1.0 - args.iou)
